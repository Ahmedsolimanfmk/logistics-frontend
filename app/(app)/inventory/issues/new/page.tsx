...
export default function NewIssuePage() {
  ...
  const onCreate = async () => {
    if (!warehouseId) {
      setToast({ open: true, message: t("issues.errWarehouse"), type: "error" });
      return;
    }
    if (!workOrderId) {
      setToast({ open: true, message: t("issues.errWorkOrder"), type: "error" });
      return;
    }
    if (!requestId) {
      setToast({ open: true, message: t("issues.errRequestId"), type: "error" });
      return;
    }
    if (reservedItems.length === 0) {
      setToast({ open: true, message: t("issues.errNoReserved"), type: "error" });
      return;
    }
            ...
            <tbody>
              {reservedItems.map((pi) => (
                <tr key={pi.id} className="border-t border-white/10">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-slate-200">{pi.internal_serial || pi.manufacturer_serial || pi.id}</td>
