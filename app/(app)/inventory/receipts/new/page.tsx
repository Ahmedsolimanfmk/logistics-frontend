...
function warehouseToken(warehouseName?: string | null) {
  ...
  return (joined || slugToken(String(warehouseName || ""), 3) || "WH").slice(
    0,
    4
  );
}
...
export default function InventoryReceiptsPage() {
  ...
  const { options: vendorOptions, loading: vendorsLoading } = useVendorOptions();
  ...
  async function searchParts() {
    ...
    try {
      ...
      setToast({
        open: true,
        message:
          e?.response?.data?.message || e?.message || "Failed to search parts",
        type: "error",
      });
    } finally {
  ...
  const serialColumns: DataTableColumn<any>[] = [
    ...
    {
      key: "serial",
      label: t("receipts.colInternalSerial") || "Internal Serial",
      render: (r) => {
        ...
        return (
          <div className="space-y-1">
            ...
            <div
              className={cn(
                "text-xs",
                isDuplicate ? "text-red-600" : "text-slate-500"
              )}
            >
    ...
    {
      key: "mfg",
      label: t("receipts.colManufacturerSerial") || "Manufacturer Serial",
      render: (r) => (
        <input
          ...
          className="trex-input w-full px-3 py-2"
        />
      ),
    },
    {
      key: "cost",
      label: t("receipts.colUnitCost") || "Unit Cost",
      render: (r) => (
        <input
    ...
    {
      key: "notes",
      label: t("receipts.colNotes") || "Notes",
      render: (r) => (
        <input
    ...
    },
    {
      ...
      label: "",
      render: (r) => (
        <Button type="button" variant="danger" onClick={() => removeRow(r.__idx)}>
          {t("receipts.remove") || "Remove"}
        </Button>
      ),
    },
  ];
    ...
    },
    {
      ...
      render: (r) => (
        <input
          ...
          onChange={(e) =>
            updateBulkRow(r.__idx, { unit_cost: e.target.value })
          }
          className="trex-input w-full px-3 py-2"
          placeholder="0.00"
        />
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <PageHeader
        title={t("receipts.newTitle") || "New Receipt"}
        actions={
          <>
            <Button type="button" variant="secondary" onClick={() => router.back()}>
              {t("common.back") || "Back"}
            </Button>
            <Button type="button" onClick={onCreate} isLoading={loading}>
              {t("receipts.createDraft") || "Create"}
            </Button>
          </>
        }
      />

      <Card title={t("receipts.header") || "Header"}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <TrexSelect
            ...
            emptyText={t("receipts.vendorsEmpty") || "لا يوجد موردون"}
          />
          ...
          <TrexInput
            ...
            placeholder={t("receipts.invoiceNoPh") || "اختياري"}
          />
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
          <div className="md:col-span-2">
            <TrexInput
              ...
              placeholder={
                t("receipts.searchPartsPlaceholder") ||
                "Search by part number / name / brand"
              }
            />
          </div>

          <div>
            <Button type="button" onClick={searchParts} isLoading={partsLoading}>
              {t("receipts.searchParts") || "Search Parts"}
            </Button>
          </div>
        </div>
          ...
          {mode === "SERIAL" ? (
            <>
              <Button
                type="button"
                variant="secondary"
                onClick={regenerateAllSerials}
              >
                {t("receipts.regenerateSerials") || "Regenerate Serials"}
              </Button>
      ...
      {mode === "SERIAL" ? (
        <DataTable
          title={t("receipts.serialItems") || "Serial Items"}
        ...
        <DataTable
          title={t("receipts.bulkItems") || "Bulk Items"}
          subtitle={`Rows: ${bulkItems.length}`}
          columns={bulkColumns}
          rows={bulkRows}
          right={
            <>
              <Button type="button" variant="secondary" onClick={addBulkRow}>
                {t("receipts.addRow") || "Add Row"}
              </Button>
              <Button type="button" onClick={onCreate} isLoading={loading}>
                {t("receipts.createDraft") || "Create"}
