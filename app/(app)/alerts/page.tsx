...
export default function AlertsPage() {
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      ...
      const qText = `${row.title} ${row.message} ${row.type} ${row.area} ${row.entity_id ?? ""}`.toLowerCase();
  ...
  const columns: DataTableColumn<AlertRow>[] = [
    ...
    },
    {
      ...
      render: (row) => (row.is_read ? t("alerts.read") || "مقروء" : t("alerts.unread") || "غير مقروء"),
    },
    {
      key: "actions",
      label: "",
    ...
    <div className="space-y-4">
      ...
      <Card title={t("alerts.filters.title") || "الفلاتر"}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div>
            <div className="mb-1 text-xs text-slate-500">{t("alerts.filters.severity") || "الأولوية"}</div>
            <select
              value={severity}
          ...
          <div>
            <div className="mb-1 text-xs text-slate-500">{t("alerts.filters.area") || "المجال"}</div>
            <select
              value={area}
          ...
          <div>
            <div className="mb-1 text-xs text-slate-500">{t("alerts.filters.read") || "القراءة"}</div>
            <select
              value={readStatus}
          ...
          <div>
            <div className="mb-1 text-xs text-slate-500">{t("common.search") || "بحث"}</div>
            <input
              value={q}
