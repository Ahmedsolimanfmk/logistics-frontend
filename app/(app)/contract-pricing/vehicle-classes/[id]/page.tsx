...
export default function Page() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const res = await contractPricingService.getVehicleClassById(id as string);
        setItem(res);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <SimpleMasterDataForm
      ...
      onSubmit={async (payload) => {
        setSaving(true);
        try {
          await contractPricingService.updateVehicleClass(id as string, {
            code: payload.code || "",
            name: payload.name,
