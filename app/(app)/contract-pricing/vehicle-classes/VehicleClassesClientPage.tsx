        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            <h2 className="text-lg font-semibold">
              {mode === "create" ? "إضافة فئة سيارة" : "تعديل فئة السيارة"}
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              إدارة أنواع وفئات السيارات المستخدمة في قواعد التسعير
            </p>
          </div>
        ...
        <form onSubmit={onSubmit} className="space-y-5 px-5 py-5">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                placeholder="وصف اختياري لفئة السيارة"
              />
            </div>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-slate-200 pt-4">
            ...
            <Button type="submit" isLoading={loading}>
              {mode === "create" ? "حفظ الفئة" : "حفظ التعديلات"}
            </Button>
          </div>
        </form>
      </div>
...
export default function VehicleClassesClientPage() {
  const [loading, setLoading] = useState(true);
  const [items, setItems] = useState<VehicleClassRef[]>([]);
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await contractPricingService.listVehicleClasses({
        q: q || undefined,
      ...
      setToast({
        type: "error",
        msg: e?.response?.data?.message || e?.message || "فشل تحميل فئات السيارات",
      });
    } finally {
      setLoading(false);
    }
  ...
  function openEdit(row: VehicleClassRef) {
    setModalMode("edit");
    setEditingId(row.id);
    setForm({
      code: row.code || "",
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const payload: VehicleClassPayload = {
      code: form.code.trim(),
      name: form.name.trim(),
    ...
    try {
      ...
      if (modalMode === "create") {
        const created = await contractPricingService.createVehicleClass(payload);
        setItems((prev) => [created, ...prev]);
        setTotal((prev) => prev + 1);
        setToast({ type: "success", msg: "تم إنشاء فئة السيارة بنجاح" });
      } else if (editingId) {
        const updated = await contractPricingService.updateVehicleClass(editingId, payload);
        setItems((prev) => prev.map((x) => (x.id === editingId ? updated : x)));
        setToast({ type: "success", msg: "تم تحديث فئة السيارة بنجاح" });
      }
      ...
      setToast({
        type: "error",
        msg: e?.response?.data?.message || e?.message || "فشل حفظ فئة السيارة",
      });
    } finally {
      setSaving(false);
    }
  ...
  async function onToggle(row: VehicleClassRef) {
    try {
      const updated = await contractPricingService.toggleVehicleClass(row.id);
      setItems((prev) => prev.map((x) => (x.id === row.id ? updated : x)));
      setToast({
        type: "success",
        msg: "تم تحديث حالة فئة السيارة بنجاح",
      });
    } catch (e: any) {
      setToast({
        type: "error",
  ...
  const columns: DataTableColumn<VehicleClassRef>[] = useMemo(
    () => [
      {
        key: "code",
        label: "الكود",
  ...
  return (
    <div className="min-h-screen space-y-6">
      <PageHeader
        title="فئات السيارات"
        subtitle="إدارة فئات السيارات المستخدمة في قواعد التسعير التعاقدي"
        actions={<Button onClick={openCreate}>+ إضافة فئة سيارة</Button>}
      />

      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="text-xs text-slate-500">إجمالي الفئات</div>
      ...
      <DataTable
        title="قائمة فئات السيارات"
        subtitle="البيانات الأساسية لفئات السيارات"
        columns={columns}
        rows={items}
        loading={loading}
        emptyTitle="لا توجد فئات سيارات"
