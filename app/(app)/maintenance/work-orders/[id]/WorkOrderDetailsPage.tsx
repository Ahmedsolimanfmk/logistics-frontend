...
export default function WorkOrderDetailsPage() {
  ...
  const hubOpenIssues = useCallback(() => {
    if (!id) return;
    router.push(`/inventory/issues?work_order_id=${encodeURIComponent(id)}`);
  }, [id, router]);

  const hubAddInstallations = useCallback(() => {
    setTabAndUrl("installations");
  }, [setTabAndUrl]);

  const hubCreateRequest = useCallback(() => {
    if (!id) return;
  ...
  }, [id, router, hubWarehouseId, showToast]);
  ...
  const installableParts = useMemo(() => {
    ...
    const rows: Array<{ part_id: string; part: any; issued_qty: number; installed_qty: number; remaining_qty: number }> =
      [];
    ...
    for (const [part_id, v] of issuedByPart.entries()) {
      ...
      if (remaining_qty > 0.0005) {
        rows.push({ part_id, part: v.part, issued_qty: v.qty, installed_qty, remaining_qty });
      }
    }
  ...
  }, [token, id, canManage, loadHubCounts, load]);

  const addIssueLine = useCallback(async () => {
    if (!token) return;
    setLineMsg(null);
  ...
  }, [token, canManage, issueId, linePartId, lineQty, lineUnitCost, lineNotes, load]);

  const addInstallation = useCallback(async () => {
    if (!token || !id) return;
    setInstMsg(null);
  ...
  }, [token, id, canManage, qaResult, qaRemarks, rs, showToast, load]);

  const completeWO = useCallback(async () => {
    if (!token || !id) return;
    setCompleteMsg(null);
  ...
  }, [token, id, canComplete, showToast, load]);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900" dir="rtl">
      <div className="space-y-4 p-4">
        ...
        <Card>
          ...
          {!loading && err ? <div className="mt-3 text-sm text-red-700">{t("common.error")}: {err}</div> : null}
        </Card>

        <Card title="WO Hub">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div className="text-xs text-slate-500">ربط المخزن + الطلبات + الصرف + التركيبات من نفس صفحة أمر العمل</div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" onClick={loadWarehouses} disabled={whLoading} isLoading={whLoading}>
                  Refresh Warehouses
                </Button>
              </div>
              ...
              <Button variant="secondary" onClick={hubOpenRequests}>
                Open Requests
                <span className="ml-2 rounded-full border border-black/10 bg-black/[0.02] px-2 py-0.5 text-xs">
              ...
              <Button variant="ghost" onClick={loadHubCounts} disabled={hubCountsLoading} isLoading={hubCountsLoading}>
                Refresh Counts
              </Button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Card title={t("woDetails.wo")}>
            <div className="grid grid-cols-1 gap-2 text-sm">
              ...
              <div><span className="text-slate-500">{t("woDetails.odometer")}:</span> {wo?.odometer ?? "—"}</div>
              <div><span className="text-slate-500">{t("woDetails.notes")}:</span> {wo?.notes || "—"}</div>
            </div>
          </Card>
            ...
            <table className="min-w-[900px] w-full text-sm">
              ...
              <tbody>
                {(recon.matched || []).map((r: any) => (
                  <tr key={`m_${r.part_id}`} className="border-t border-black/10">
                    <td className="p-3">
                    ...
                    <td className="p-3"><span className="text-green-700">{t("woDetails.matched")}</span></td>
                  </tr>
                ))}

                {(recon.issued_not_installed || []).map((r: any) => (
                  <tr key={`i_${r.part_id}`} className="border-t border-black/10">
                    <td className="p-3">
                      <div className="font-semibold">{r?.part?.name || "—"}</div>
                      <div className="text-xs text-slate-500 font-mono">{shortId(r.part_id)}</div>
                    </td>
                    ...
                    <td className="p-3">{r.installed_qty}</td>
                    <td className="p-3">{r.issued_cost}</td>
                    <td className="p-3"><span className="text-amber-700">{t("woDetails.issuedGtInstalled")}</span></td>
                  </tr>
                ))}

                {(recon.installed_not_issued || []).map((r: any) => (
                  <tr key={`x_${r.part_id}`} className="border-t border-black/10">
                    <td className="p-3">
                      <div className="font-semibold">{r?.part?.name || "—"}</div>
                      <div className="text-xs text-slate-500 font-mono">{shortId(r.part_id)}</div>
                    </td>
                    ...
                    <td className="p-3">{r.installed_qty}</td>
                    <td className="p-3">—</td>
                    <td className="p-3"><span className="text-red-700">{t("woDetails.installedGtIssued")}</span></td>
                  </tr>
                ))}

                {(recon.matched || []).length + (recon.issued_not_installed || []).length + (recon.installed_not_issued || []).length === 0 ? (
                  <tr className="border-t border-black/10">
                    <td className="p-3 text-slate-500" colSpan={5}>{t("woDetails.noReconData")}</td>
                  </tr>
                ) : null}
              </tbody>
            </table>
        ...
        {tab === "issues" ? (
          ...
          >
            <div className="text-xs text-slate-500">
              ...
              <span className="font-mono text-[rgb(var(--trex-fg))]">{issueId ? shortId(issueId) : "—"}</span>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
              <div className="md:col-span-2">
                <div className={labelCls}>{t("woDetails.partIdUuid")}</div>
                <input
                  value={linePartId}
              ...
              <div>
                <div className={labelCls}>{t("woDetails.qty")}</div>
                <input type="number" value={lineQty} onChange={(e) => setLineQty(Number(e.target.value))} className={inputCls} />
              </div>

              <div>
                <div className={labelCls}>{t("woDetails.unitCost")}</div>
                <input type="number" value={lineUnitCost} onChange={(e) => setLineUnitCost(Number(e.target.value))} className={inputCls} />
              </div>

              <div className="md:col-span-4">
                <div className={labelCls}>{t("woDetails.notesOpt")}</div>
                <input value={lineNotes} onChange={(e) => setLineNotes(e.target.value)} className={inputCls} placeholder={t("common.optional")} />
              </div>
              ...
              <table className="min-w-[900px] w-full text-sm">
                ...
                <tbody>
                  {issuedLines.length === 0 ? (
                    <tr className="border-t border-black/10">
                      <td colSpan={6} className="p-3 text-slate-500">{t("woDetails.noIssuedLines")}</td>
                    </tr>
                  ) : (
                    issuedLines.map((l: any, idx: number) => (
                      <tr key={`${l.issue_id}_${l.part_id}_${idx}`} className="border-t border-black/10">
                        <td className="p-3">
        ...
        {tab === "installations" ? (
          ...
          >
            {instMsg ? <div className="mt-2 text-xs text-red-700">⚠ {instMsg}</div> : null}

            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
              <div className="md:col-span-2">
                <div className={labelCls}>القطعة (من المصروف لهذا أمر الشغل)</div>
                <select
                  value={instPartId}
                  onChange={(e) => {
              ...
              {serialOptions.length > 0 ? (
                <div className="md:col-span-2">
                  <div className={labelCls}>Serial (للقطع المسلسلة)</div>
                  <select value={instPartItemId} onChange={(e) => setInstPartItemId(e.target.value)} className={inputCls}>
                  ...
                  <div className="mt-1 text-xs text-slate-500">ملاحظة: في حالة السيريال، الكمية = 1 تلقائيًا.</div>
                </div>
              ) : null}

              <div>
                <div className={labelCls}>{t("woDetails.qtyInstalled")}</div>
                <input
                  type="number"
                  value={instQty}
                ...
                {serialOptions.length > 0 || Boolean(instPartItemId) ? (
                  <div className="mt-1 text-xs text-slate-500">تم تعطيل الكمية لأن القطعة Serial.</div>
                ) : null}
              </div>

              <div>
                <div className={labelCls}>{t("woDetails.odometerOpt")}</div>
                <input
                  type="number"
                  value={instOdo}
              ...
              <div className="md:col-span-4">
                <div className={labelCls}>{t("woDetails.notesOpt")}</div>
                <input value={instNotes} onChange={(e) => setInstNotes(e.target.value)} className={inputCls} placeholder={t("common.optional")} />
              </div>
            ...
            <div className="mt-2 overflow-auto rounded-xl border border-black/10">
              <table className="min-w-[900px] w-full text-sm">
                ...
                <tbody>
                  {instLoading ? (
                    <tr className="border-t border-black/10">
                      <td colSpan={5} className="p-3 text-slate-500">{t("common.loading")}</td>
                    </tr>
                  ) : instItems.length === 0 ? (
                    <tr className="border-t border-black/10">
                      <td colSpan={5} className="p-3 text-slate-500">{t("woDetails.noInstallations")}</td>
                    </tr>
                  ) : (
                    instItems.map((x: any) => (
              ...
              <table className="min-w-[900px] w-full text-sm">
                ...
                <tbody>
                  {installationsRuntime.length === 0 ? (
                    <tr className="border-t border-black/10">
                      <td colSpan={5} className="p-3 text-slate-500">{t("woDetails.noRuntimeInstallations")}</td>
                    </tr>
                  ) : (
        ...
        {tab === "qa" ? (
          <Card title={t("woDetails.qaTitle")}>
            {!canManage ? (
              <div className="text-sm text-slate-600">{t("woDetails.onlyAdminAccountantQa")}</div>
            ) : (
              <div className="space-y-3">
                ...
                <div>
                  <div className={labelCls}>{t("woDetails.roadTestResult")}</div>
                  <select value={qaResult} onChange={(e) => setQaResult(e.target.value as any)} className={inputCls}>
                ...
                <div>
                  <div className={labelCls}>{t("woDetails.remarks")}</div>
                  <textarea
                    value={qaRemarks}
