...
export default function WorkOrderDetailsPage() {
  ...
  // ✅ Tab from URL
  const tabFromUrl = String(sp?.get("tab") || "").toLowerCase();
  const initialTab: TabKey =
    tabFromUrl === "installations"
      ? "installations"
      : tabFromUrl === "qa"
      ? "qa"
      : "issues";
  ...
  // ✅ WO Hub: warehouse selector for Create Request
...
const [hubCountsLoading, setHubCountsLoading] = useState(false);
  ...
  // ✅ keep tab synced to URL (without losing other params)
  ...
  function hubCreateRequest() {
    ...
    // warehouse_id مهم هنا عشان الطلب يبقى مربوط بمخزن
  ...
  try {
    // Requests count
    const reqRes: any = await apiFetch(`/inventory/requests?work_order_id=${encodeURIComponent(id)}&page=1&limit=1`, { token });
    const requestsTotal = pickTotal(reqRes);

    // Issues count
    const issRes: any = await apiFetch(`/inventory/issues?work_order_id=${encodeURIComponent(id)}&page=1&limit=1`, { token });
    const issuesTotal = pickTotal(issRes);

    // Installations count (من endpoint الحالي)
    ...
    // لو endpoint من دول مش موجود عندك لسه، مش هنعطّل الصفحة
    // هنسيب الأعداد زي ما هي
  } finally {
  ...
  async function loadInstallations() {
    ...
    try {
      const res: any = await apiFetch<any>(`/maintenance/work-orders/${id}/installations`, {
        token,
      });
  ...
  Refresh Counts
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <div className="text-sm">
                  <span className="text-white/60">{t("woDetails.completedAt")}:</span>{" "}
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                {issueMsg ? (
                  <div
                    className={cn(
                      "mt-2 text-sm",
                      issueMsg.startsWith("✅") ? "text-green-200" : "text-red-200"
                    )}
                  >
                    {issueMsg}
                  </div>
                  ...
                  <div className="md:col-span-4 flex items-center gap-2">
                    ...
                    {lineMsg ? (
                      <div
                        className={cn(
                          "text-sm",
                          lineMsg.startsWith("✅") ? "text-green-200" : "text-red-200"
                        )}
                      >
                        {lineMsg}
                      </div>
                    ...
                    <tbody>
                      {issuedLines.length === 0 ? (
                        ...
                        issuedLines.map((l: any, idx: number) => (
                          <tr
                            key={`${l.issue_id}_${l.part_id}_${idx}`}
                  ...
                  <div className="md:col-span-4 flex items-center gap-2">
                    ...
                    {instMsg ? (
                      <div
                        className={cn(
                          "text-sm",
                          instMsg.startsWith("✅") ? "text-green-200" : "text-red-200"
                        )}
                      >
                        {instMsg}
                      </div>
